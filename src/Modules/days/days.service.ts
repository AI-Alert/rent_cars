...
import { THREE_DAYS_IN_MILISECOND, ONE_DAY_IN_MILISECOND, DAYS_IN_NUMBER, DISCOUNT_PRICE, MAXIMUM_RENT_DAY_LIMIT, BASEPRICE } from '../estimates/estimates';
...
export class DaysService {
    constructor(private readonly rentDayRepository: DaysRepository, private readonly autoRentService: CarsService) { }
    ...
    async getInfoPossibleToRent(carId: number, date: RequestDateDto): Promise<ResponseMessage | BadRequestException> {
        const possibleTorent = await this.getIsPossibleToRent(carId, date)
        if (!possibleTorent) {
    ...
    async rentAutoCalculation(carId: number, date: RequestDateDto): Promise<ResponseMessage | BadRequestException> {
    ...
    async rentAutoSession(createRentDto: CreateRentDto): Promise<ResponseMessage | BadRequestException> {
        ...
        const isPossibletoRent = await this.getIsPossibleToRent(createRentDto.carId, date)
    ...
    async getIsPossibleToRent(carId: number, date: RequestDateDto): Promise<boolean> {
        ...
        const auto = await this.autoRentService.findById(carId)
        if (!auto) {
            throw new BadRequestException('There is no auto with that ID ')
        }
        const rentedMonths = await this.rentDayRepository.findRentMonthByAutoId(carId, newDate)
    ...
    async createRentDaySession(createRentDto: CreateRentDto): Promise<ResponseMessage | BadRequestException> {
        const auto = await this.autoRentService.findById(createRentDto.carId)
        if (auto) {
            const data: DaysEntity = {
                carId: auto.car_id,
                fromDate: this._dateYYYYMMDDFormatter(createRentDto.fromDate),
                toDate: this._dateYYYYMMDDFormatter(createRentDto.toDate)
            }
            const entity = new DaysEntity(data)
